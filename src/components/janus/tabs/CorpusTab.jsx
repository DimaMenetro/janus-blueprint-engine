      ...
      {data.constraints && data.constraints.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="font-medium text-slate-900">Constraints</h4>
          ...
          <ul className="space-y-2">
            {data.constraints.map((constraint, idx) => (
              <li key={idx} className="flex items-start gap-3 text-sm">
                ...
                <span className="text-slate-700 pt-0.5">{constraint}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {data.feasibility_notes && data.feasibility_notes.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="font-medium text-blue-900">Feasibility Notes</h4>
          </div>
          <ul className="space-y-2">
            {data.feasibility_notes.map((note, idx) => (
              <li key={idx} className="text-sm text-blue-800 pl-4 border-l-2 border-blue-300">
                {note}
              </li>
            ))}
          </ul>
